...
static void GetIf(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        int res;
        if (auto item1 = std::get_if<One>(package)) {
            res = item1->get();
        }
        else if (auto item2 = std::get_if<Two>(package)) {
            res = item2->get();
        }
        else if (auto item3 = std::get_if<Three>(package)) {
            res = item3->get();
        }
        else if (auto item4 = std::get_if<Four>(package)) {
            res = item4->get();
        }
